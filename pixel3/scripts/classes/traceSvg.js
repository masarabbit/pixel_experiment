...
class TraceSvg {
  constructor(props) {
    ...
    settings.inputs.dataUrl.value = this.pathData.map(p=> {
      return `<path fill="${p.color}" d="${p.path.join(' ')}" ${p.isEvenOdd ? 'fill-rule="evenodd"' : ''}/>`
    }).join(' ').replaceAll('ffffff','fff').replaceAll('000000','000').replaceAll('*','transparent')
  }
  checkSurroundingCells = index => {
    return !this.arr[index + this.checkDirection[this.cI]] || // cell in the check direction is the edge
  ...
  recordTraceData = (cItoCheck, index, path) =>{
    ...
    if (this.proceed && !this.checkedIndex.some(i => i === cItoCheck) && this.checkSurroundingCells(index)){
      ...
      if (this.pos.h === this.pos.x && this.pos.v === this.pos.y) this.proceed = false
  ...
  convertToSvg(colors){  
    ...
    while (colors.some(c => c !== '')) {
      ...
      Object.assign(this.pos, { x: this.calcX(first), h: this.calcX(first), y: this.calcY(first), v: this.calcY(first) })
